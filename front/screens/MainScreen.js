...
export default function MainScreen() {
  ...
  /** ⬇️ 추가: 오늘만 필터링 (UI/텍스트 변경 없음) */
  /** 오늘 요일(KO) */
  const todayKo = useMemo(() => {
    const d = new Date();
    return d.getDay() === 0 ? '일' : daysKo[d.getDay() - 1];
  }, []);
  ...
  /** 오늘 일정만 (현재 주차일 때만 노출) */
  const todayStudySchedules = useMemo(() => {
    if (!isCurrentWeek) return [];
    return studySchedules.filter(s => s.day === todayKo);
  }, [studySchedules, todayKo, isCurrentWeek]);
